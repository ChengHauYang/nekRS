#!/usr/bin/env python3
"""Convergence study driver for the sphereIBM example.

Automates multi-parameter convergence sweeps for the nekRS immersed boundary
method implementation applied to the lid-driven-cavity sphere case.  Each
sweep varies a single parameter while holding all others at their default
values from sphereIBM.par.

Supported sweeps
----------------
- polynomialOrder : N = 3, 5, 7, 9
- dt              : 4e-3, 2e-3, 1e-3, 5e-4
- markerSpacing   : 0.08, 0.04, 0.02, 0.01
- gaussianWidth   : 0.08, 0.04, 0.02, 0.01
- supportRadius   : 0.16, 0.08, 0.04, 0.02
- shellThickness  : 0.08, 0.04, 0.02, 0.01
...
# Default parameter values matching sphereIBM.par
...
def generate_par_content(overrides: Dict[str, float]) -> str:
    """Return the full text of a sphereIBM.par with *overrides* applied.
    ...
    # Use a short endTime for convergence studies (CI-like mode)
    end_time = 0.004
...
[CASEDATA]
stl_file = geometry/sphere_r025.stl
...
# Pattern for the sphereIBM diagnostics line printed by UDF_ExecuteStep:
#   sphereIBM step=... time=... slipMax=... slipRms=... force=(...)
_RE_DIAGNOSTICS = re.compile(
    r"sphereIBM\s+step=\d+\s+time=[\d.eE+\-]+\s+"
...
def parse_stdout(stdout: str) -> RunResult:
    ...
    For the sphere case the diagnostics are printed at every checkpoint /
    final step.  We take the *last* occurrence so that we capture the
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run IBM convergence sweeps for sphereIBM.",
